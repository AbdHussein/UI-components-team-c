...
import {Animation, Iprops, Variant } from "./Index";
   ...
   width:${(props:Iprops) => handleWidth(props.width)};
   height:${(props:Iprops) => handleHeight(props.height)};
   background:${(props:Iprops)=>props.bgColor?props.bgColor:'#D3DEDC'};
   border-radius: ${(props:Iprops) => handleBorderRadius(props.variant)};
   animation ${(props:Iprops) => handleAemation(props.animation)} 2s linear 2s infinite;
   margin-top:10px;
   `;
